<# 
 ...
 .NOTES 
	...
	DateUpdated: 2014-Sep-18
	Version: 1.1.1
	Limitations: 	Doesn't cover what it doesn't cover (replication, linked servers, etc)
...
All databases, logins, job objects and sp_configure options will be migrated from sqlserver\instance to sqlcluster. Databases will be migrated using the detach/copy files/attach method. DBowner will be updated. User passwords, SIDs, database roles and server roles will be migrated along with the login.
 ...
 .EXAMPLE   
.\Start-SQLMigration.ps1 -Source sqlserver\instance -Destination sqlcluster -AllUserDBs -ExcludeDBs Northwind, pubs -IncludeSupportDBs -force -AllUsers -ExcludeLogins nwuser, pubsuser, "corp\domain admins"  -MigrateJobServer -ExportSPconfigure
...
Function Copy-SQLDatabases  {
 <#
	...
	If (!(Test-Path (Join-AdminUNC $sourcenetbios $sourceserver.databases['master'].PrimaryFilePath))) { 
		Write-Host "Can't access remote SQL directories on $source. Halting database migration." -ForegroundColor Red
		return 
	}
	
	If (!(Test-Path (Join-AdminUNC $destnetbios $destserver.databases['master'].PrimaryFilePath))) { 
		...
		if ($BackupRestore) {
			If ($Pscmdlet.ShouldProcess($destination,"Backup $dbname from $source and restoring.")) {
				...
				if ($result -eq $true) {
					...
					If ($result) {
						Add-Content -Path "$csvfilename-dbowner.csv" "$($destdb.name),$dbowner"
					}
				} else { $skippedb[$dbname] = $result }
			}
		} # End of backup
	...
	$skippedb.GetEnumerator() | Sort-Object Value | Select Name, Value | Export-Csv -Append -Path "$csvfilename-db.csv" -NoTypeInformation
	...
	foreach ($sourcelogin in $sourceserver.logins) {
			...
			if ($username -eq $destserver.ServiceAccount) { Write-Warning "$username is the destination service account. Skipping drop."; continue }
				If ($Pscmdlet.ShouldProcess($destination,"Dropping $username")) {
					...
					try {
						...
						Write-Host "Could not drop $username. Skipping" -ForegroundColor Red
						continue }
				}
		}
		...
		If ($Pscmdlet.ShouldProcess($destination,"Adding SQL login $username")) {
			...
			if ($sourcelogin.LoginType -eq "SqlLogin") {
				...
				$ds = $sourceserver.databases['master'].ExecuteWithResults($sql)
				$hashedpass = $ds.tables[0].rows[0].Item("hashedPass")
				...
				try {
					...
					try {
						...
						$null = $destserver.databases['master'].ExecuteNonQuery($sqlfailsafe) 
						...
						Write-Host "Failed to add $username to $destination. See log for details." -ForegroundColor Red
						continue 
					}
				}
			}
			...
			elseif ($sourcelogin.LoginType -eq "WindowsUser" -or $sourcelogin.LoginType -eq "WindowsGroup") {
				...
				try {
					...
					Write-Host "Failed to add $username to $destination" -ForegroundColor Red
					continue }
			}
				...
				if ($destserver.roles[$role.name] -ne $null) { 
					If ($Pscmdlet.ShouldProcess($destination,"Adding $username to $($role.name) server role")) {
						try {
							...
							Write-Host "Failed to add $username to $($role.name) server role."  -ForegroundColor Red }
							}
						}
					}
				}
			...
			foreach ($perm in $perms) {
				...
				If ($Pscmdlet.ShouldProcess($destination,"Performing $permstate on $($perm.permissiontype) for $username")) {
					try { 
						...
						Write-Host "Failed to $permstate $($perm.permissiontype) to $username"  -ForegroundColor Red
					}
				}
			}
				...
				if ($destserver.Credentials[$credential.name] -eq $null) {
					If ($Pscmdlet.ShouldProcess($destination,"Adding $($credential.name) to $username")) {
						try {
							...
							Write-Host "Failed to $permstate $($perm.permissiontype) to $username on $destination"  -ForegroundColor Red }
					}
				}
			}
		}
			...
			if ($destdb -ne $null) {
				if ($destdb.users[$dbusername] -eq $null) {
					If ($Pscmdlet.ShouldProcess($destination,"Adding $dbusername to $dbname")) {
						...
						catch { Write-Host "Failed to add $dbusername ($dblogin) to $dbname on $destination." -ForegroundColor Red 
						}
					}
				}	
			 # DB owner
				If ($sourcedb.owner -eq $username) {
					If ($Pscmdlet.ShouldProcess($destination,"Changing $dbname dbowner to $username")) {
						try {
							...
							} else { Write-Host "Failed to update $($destdb.name) owner to $($sourcedb.owner)." -ForegroundColor Red }
						} catch { Write-Host "Failed to update $($destdb.name) owner to $($sourcedb.owner)." -ForegroundColor Red }
					}
				}
			 ...
			 # Database Roles: db_owner, db_datareader, etc
					...
					if ($role.EnumMembers() -contains $username) {
						...
						if ($destdbrole -ne $null -and $dbusername -ne "dbo" -and $destdbrole.EnumMembers() -notcontains $username) { 
							If ($Pscmdlet.ShouldProcess($destination,"Adding $username to $($role.name) database role on $dbname")) {
								...
								$destdb.Alter() }
								catch { Write-Host "Failed to add $username to $($role.name) database role on $dbname." -ForegroundColor Red }
							}
						}
					}
				}
				...
				foreach ($perm in $perms) {
					...
					If ($Pscmdlet.ShouldProcess($destination,"Performing $permstate on $($perm.permissiontype) for $username on $dbname")) {
						...
						catch { Write-Host "Failed to perform $permstate on $($perm.permissiontype) for $username on $dbname." -ForegroundColor Red }		
					}
				}
			}
		}
...
Function Export-SQLSPConfigure     {
 <#
	...
	$server.databases['master'].ExecuteNonQuery("RECONFIGURE WITH OVERRIDE")
...
#>
		...
		If ($Pscmdlet.ShouldProcess($destination,"Execute sp_configure")) {
			...
			$sourceserver.databases['master'].ExecuteNonQuery("RECONFIGURE WITH OVERRIDE")
			$destserver.Configuration.ShowAdvancedOptions.ConfigValue = $true
			$destserver.databases['master'].ExecuteNonQuery("RECONFIGURE WITH OVERRIDE")
	...
	foreach ($jobobject in $jobobjects) {
		...
		If ($Pscmdlet.ShouldProcess($destination,"Adding $jobobject $agentname")) {
			Write-Host "Attempting migration of $jobobject $agentname" -ForegroundColor Yellow
				try {
				$sql = $agent.script()	
				$null = $destserver.databases['master'].ExecuteNonQuery($sql)
...
Function Update-SQLdbowner  { 
		...
		try {
			...
			Write-Host "Changed $dbname owner to $dbowner." -ForegroundColor Cyan
...
Function Update-SQLdbReadOnly  { 
		...
		try {
			$null = $server.databases['master'].ExecuteNonQuery($sql)
			Write-Host "Changed ReadOnly status to $readonly for $dbname on $($server.name)." -ForegroundColor Cyan
			return $true
		} catch { 
	...
	foreach ($systemdb in $systemdbs) {
		...
		Write-Host "Migrating user objects in $systemdb" -ForegroundColor Yellow
			...
			foreach ($query in $sqlQueries) {
				...
				} catch {} # Do nothing. Usually if it fails, it's because a user objcet already exists. No bigs.
			}
	}
	return $true
}
...
PROCESS {
	...
	$sourcefilestream = $sourceserver.databases['master'].ExecuteWithResults($sql)
	$destfilestream = $destserver.databases['master'].ExecuteWithResults($sql)
	if ($sourcefilestream.tables.fs -gt 0 -and $destfilestream.tables.fs -eq 0)  { $fswarning = $true }
